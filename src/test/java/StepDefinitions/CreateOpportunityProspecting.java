...
public class CreateOpportunityProspecting extends BaseTest{
CreateSalesNewOpportunity createNewOpportunity;
OpportunityDetailsTab oppDetails;
String stagename;
String probability;

//create new opportunity with prospecting stage
...
createNewOpportunity.createNewOpp(
		...
		getParameters().getProperty("OPP_STAGE_PROSPECTING")
		);
}

//validate the stage and probability on opportunity details tab
@Then ("verify opportunity details for stage prospecting")
public void verify_opportunity_details_for_stage_prospecting() throws InterruptedException, IOException 
{		
	...
	Assert.assertEquals(stagename, getParameters().getProperty("OPP_STAGE_PROSPECTING"));
	Assert.assertEquals(probability, getParameters().getProperty("OPP_PROBABILITY_PROSPECTING"));
